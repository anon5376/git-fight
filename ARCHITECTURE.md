...
`your_role` is `ours`, `theirs`, `both` (mirror), or `spectator`. Spectators never have a fighter slot. Outbound Tick/Hash/End/Hello (including join Hello/Snapshot and closed-room Error) are non-blocking so a client who stops reading cannot stall confirm; they resync from Snapshot on reconnect. Join Hello/Snapshot and the next-round Hello are queued if the outbound channel is full.
