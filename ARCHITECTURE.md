...
Record `pr_head_sha` and `pr_base_sha` on the match **before** clone so a second `/fight` or a `synchronize` during git work still sees the open match. If clone or hunk collection fails, status becomes `aborted` and another `/fight` can start. A busy abort write retries once, then a short background retry, instead of treating SQL error as already-closed (which would leave the pending row and block rematch `/fight`). The 5s expirer also aborts a GitHub `pending`/`in_progress` row that still has no hunks after the 120s git job window, so a leftover clone abort cannot hold `matches_one_open_per_pr`. That backstop does not abort a clone that is still in flight. Milestone 5 refuses to push if either SHA has moved.
...
Map commit author emails to GitHub logins through the commits API. Same person on both sides: a **mirror match** (that login may occupy both slots; 2-player keys like the CLI). No GitHub account for the other author: the CPU plays them under their name. A blamed or stored login that is not a GitHub name is the same as no account — dropped, not kept as a human slot that nobody can occupy. If the blamed commit already has an `author.login` that is not a GitHub name, do not fall back to `?author=email` (that list is any recent commit with the address, not this SHA). A transient commits-API failure is not “no account”: the challenge does not start (quiet abort) so rematch `/fight` can look the login up again. If blamed authors differ across hunks, the right-side identity (and who may send right-side inputs) updates at round start. A hunk with no blamed GitHub login is CPU for that round only; it does not inherit the previous hunk's human login.

### 6. Challenge comment

Insert the match (status `pending`, `expires_at` = now + 24 hours, random seed, stored hunks). Comment who is fighting, how many rounds, and `https://<public-host>/match/<id>`. If that row is aborted or expired before clone finishes or before the challenge comment is posted (PR moved, 24h), do not post a fight link or a second “could not start”. The final open-status read before returning that link is fail-closed: only `Ok(true)` posts (`Err` is silent, no `match_id`). After clone, a busy open-status read still continues if hunks exist. Hunk rows are inserted in one transaction so a failed later hunk cannot leave a partial open match. That insert (and the fighter-row update) is open-status only: an aborted or expired row cannot gain hunks after SHA-drift or clone abort. The closing latch is the same fail-closed gate as Join: clone-finish and the expirer re-check that latch and open status immediately before POST so a `synchronize` cannot sneak a match URL out after clients already see `outdated`. A poisoned closing lock blocks the fight-link POST and sends Join `preparing` (reconnect), not terminal `outdated` for every match. `challenge_comment_id` is write-once when NULL, including after the row has already finished or aborted, so a POST that won the last-millisecond gate still gets a PATCH target. A leftover open row with hunks and no `challenge_comment_id` is posted by the 5s expirer, which skips in-flight clone comments, re-checks open status immediately before POST, and retries a posted id that has not been stored yet (SET, not a second POST). An `Ok(false)` SET on a still-NULL row stays queued; it does not dequeue as if the id were durable. Outcome comments edit that id when set; a failed PATCH does not POST a second thread. Result and skip comments are posted before `result_branch` / skip `abort_reason` so a GitHub blip stays unpublished and retries. Challenge-comment retries use the same seven-step backoff as abort.

### 7. Play

Players log in with GitHub. Only the two fighter logins can take a slot; everyone else spectates. Seed, stats, and hunk metadata go to every client. Lockstep runs as in [Netcode](#netcode). Each round's winner is `ours`, `theirs`, or `draw`. A fighter who disconnects has 30 seconds to rejoin, then loses the **current** round. If a fighter never shows up, the match expires at 24 hours with no result and no push. That includes a later-round blamed author who has not occupied the right-side slot yet: the 30-second clock does not start until they have been seen, so a multi-author PR does not auto-forfeit the next conflict 30 seconds after the previous KO. An aborted or expired row stops confirming ticks on the next sim step, even if Shutdown is still queued. `--instant` re-checks before each step in a burst. A round that already ended in the sim does not write End or a hunk winner after the row is closed. That open-status read is `status` only. If the winner write fails while the row is still open and that hunk exists with no winner yet, the room retries instead of broadcasting End or starting the next conflict. A match with no hunk row (local demo) still ends. A disconnect forfeit keeps the `forfeit_*` tag across that retry so a busy SQLite write cannot become a KO pick. That tag is written before End so a restart cannot replay the round as a KO pick. `forfeit_pending` is set only after that write succeeds, so a persist miss plus reconnect cannot tag a later KO as `forfeit_*`. Once the 30-second timer fires, `forfeit_due` stays latched: a reconnect clears `disconnected_at` but cannot cancel persist retry. Write-once resume records leaderboard stats from the stored hunk `winner` / `is_ko`, not a later sim KO or a cleared `forfeit_pending`. An open-status read error is not a closed match: the room retries instead of expiring or skipping a live fight. A Join while that read is busy gets Error { preparing } (no Hello/Snapshot) so clients do not lockstep against a row that may already be aborted. A busy match-row or session-cookie read on `GET /ws` is the same preparing Error, not `not found` or a spectator Hello, so a SQLite blip cannot lock a fighter out or start the 30-second clock as a ghost spectator. A busy `pending` → `in_progress` write does not expire the row; join and the clock retry the guarded start. After clone, a busy open-status read still posts the challenge if hunks exist. The last-round `finished` write retries the same way so a won fight cannot sit `in_progress` until the 24-hour expiry buries it. A scored-open resume uses that same retry: a busy `finished` mark does not stop the room. Last-round `End { match_over }` is sent only after the row is `finished`; an aborted or expired row (SHA-drift, 24h) gets the terminal Error instead of a replay URL. If another writer already marked the row `finished` while this room still has a last-round `sim.result`, existing sockets still get that `End`, not `Error { finished }`. A fully scored open match is not expired: the live room clock, `expire_open_match`, and `expire_pending` skip those rows; boot, the 5s expirer, and the live room mark them finished and publish. A room that started before the last winner was stored re-reads hunks after that expire no-op so it can mark `finished` instead of skipping last-round finish. Leaderboard writes are claim-once per round (`stats_recorded`) so that path can record a missing round without double-counting. The claim and the `player_stats` deltas commit together so a failed leaderboard write can retry. A KO is stored on the hunk (`is_ko`) so a resume after `finished` still records that KO once, never after abort or expiry. Boot migrate must not mark unrecorded winners as recorded. Hello and Snapshot clip the hunk path so a 4096-byte name cannot bloat the room. The canvas drops held buttons when Hello starts the next round so a leftover key from the KO cannot confirm after `next_tick` resets. A non-final End also advances the Input `round` and resets the send cursor so a late or dropped next-round Hello cannot leave those buttons tagged for the finished conflict (`--instant` does not idle-confirm). A non-final End is broadcast only after a successful open-status read so a busy SQLite check cannot leave clients on the next Input `round` while the room stays on this conflict. The last-round `End` (`match_over`), a non-final `End`, and expiry `Error` are queued if the outbound channel is full — clients advance Input `round` on `End`, and Snapshot ticks do not recover a missed inter-round `End`. A non-final `End` and the next-round Hello are sent as one ordered pair on that socket so a full outbound buffer cannot deliver Hello first. Spectator `Error { busy }` retries like `preparing` (no desync reload) so a teammate watching is not kicked off when Join cannot enter the room queue. Confirmed ticks are written to `match_inputs` before the sim steps or the Tick is broadcast, so a busy SQLite write cannot leave lockstep ahead of the replay log. A transient blamed-author HTTP miss aborts the pending row instead of storing CPU / `theirs_login = NULL`, so a GitHub blip cannot lock the right-side slot to the computer. GitHub-match Input must carry the Hello `round`; omitted `round` is dropped so a leftover KO press cannot confirm after `next_tick` resets. A later-round blamed author cannot put Input on the room queue until their conflict is the current round. A busy `current_theirs_login` read retries once, then reloads hunks so a later-round advance cannot leave the cache on the previous author. Only if that hunks read is also busy does the socket keep the last good login, so a same-round SQLite blip cannot drop the current-round theirs. Spectator Input is still dropped before the queue. Fighter `Leave` is `try_send` (full = spawned) so a full room queue cannot stall the 30-second disconnect clock.
...
- Otherwise build each resolved file in core from the winning side. `git hash-object -w` the blobs, a temporary index, `write-tree`, `commit-tree` with parents `(pr_head_sha, pr_base_sha)`. Commit message lists each round and who won it. Push **only** `refs/heads/git-fight/pr-<number>-<match-id>` (create, never `--force`). If that ref already points at this match's commit (author `git-fight`, those parents, message `git fight match <id>`), that is success — crash recovery, not overwrite. That is only after a successful `cat-file` of the advertised commit. An advertised `git-fight/*` ref that cannot be fetched, or a commit that cannot be read, is a transient push failure — not an `exists` skip. A create-only push that then sees the ref appear re-inspects the same way. Resolve ignores a `hunk_index` outside `0..15` so a hostile row cannot allocate on publish.
...
On WebSocket connect the server reads the session. A busy session or match-row read is Error { preparing } so the canvas reconnects; a missing cookie or expired session is spectator. On success it sends:

```text
Hello { match_id, seed, input_delay, your_role, ours, theirs, round, confirmed_tick, you_are, path, hunk_index }
```

`your_role` is `ours`, `theirs`, `both` (mirror), or `spectator`. Spectators never have a fighter slot. Outbound Tick/Hash/End/Hello (including join Hello/Snapshot and closed-room Error) are non-blocking so a client who stops reading cannot stall confirm; they resync from Snapshot on reconnect. Join Hello/Snapshot, the next-round Hello, last-round End, a non-final End, and expiry Error are queued if the outbound channel is full. Inter-round End is enqueued ahead of that Hello on the same socket.
...
Anything else is dropped. Spectators' `Input` messages are dropped before the room queue. That includes a later-round blamed author during an earlier conflict, and a previous-round theirs after their hunk is scored: only the current-round fighter logins (ours + this hunk's theirs) may enqueue. Spectator `Join` is `try_send` (full = reject) so a connect flood cannot fill that queue either. The current-round theirs is re-read on each Input so a later-round author who connected as a spectator can play when their conflict starts, without filling the queue before then. A failed re-read retries once, then reloads hunks so a later-round advance cannot leave the cache on the previous author. Only if that hunks read is also busy does the socket keep the last good login, so a busy SQLite cannot idle the current-round theirs. Connect seeds that cache from the hunks already loaded when the scalar read fails, so the current-round theirs is not treated as a spectator on the first Input. Fighter `Leave` is `try_send` (full = spawned) so that flood cannot stall the disconnect clock either.
...
to every client, including spectators. Everyone, server included, applies those two inputs and steps one tick. A failed write does not consume the pending buttons or advance `confirmed_tick`; the next clock retries. A closed match that cannot store the tick expires instead of confirming. The server may send `Hash { n, state_hash }` so a desynced client can see it is wrong. After Hello, the server sends `Snapshot { round, seed, confirmed_tick, stats, ticks, path, hunk_index }` with the confirmed input log for the current round so a joiner or reconnect can catch up in one message. Hello and Snapshot are one ordered pair on that socket so a full outbound buffer cannot deliver Snapshot first. Catch-up does not send End — `finish()` is the only source of End (`match_over` only when finished; inter-round End then Hello). A desynced client does not get to overrule the server; it should resync from that snapshot (reconnect) or reload.

`--lag-ms` delays that broadcast (and inbound `Input`) by the requested milliseconds.

### Disconnect, join, expiry

- A fighter WebSocket drop starts a 30-second rejoin timer **after that login has occupied the slot**. `Leave` is queued without blocking the read task, so a full room channel cannot keep the slot “present”. Reconnect with the same login resumes the slot. If the timer fires, they lose the current round (`ours` or `theirs` KO). That forfeit stays due while the hunk write retries — reconnect after the timer does not clear it. Later rounds can still be played if they return. A blamed author who has not been seen on the current slot is treated as never-showed-up (wait until `expires_at`), not as a disconnect. The clock does not start merely because the match is already `in_progress`.
- Status stays `pending` until both human fighters have occupied their slots at least once (CPU slots count as present). That flip is a guarded `pending` → `in_progress` update, so a join cannot un-expire, un-abort, or un-finish a row. A busy start write is `Error { preparing }` (no Hello) so the canvas reconnects; the 30 Hz loop retries the same write. If that never happens, at `expires_at` (24 hours) the match becomes `expired`, the room closes, no result, no push. A match that did start (`in_progress`) also expires at `expires_at` if it is still open, so an abandoned fight cannot hold the one-open-match slot or keep a room spinning. That flip is one `UPDATE … RETURNING`; a fight that finishes on the deadline is not listed as expired and does not get an expired comment. An open match whose every hunk already has a winner is finished and published instead of expired, so a crash after the last write cannot bury a won fight at 24 hours. The live room clock and `expire_open_match` use that same rule: a scored-all row stays open until `finished` lands. The live room is closed before the expiry comment HTTP so GitHub latency cannot keep confirming ticks on an expired row. `expire_now` retries a busy match-row read: a no-op expire (already closed, or scored-all) does not treat `Err` as vanished or as scored-all — only a known `expired`/`aborted`/`finished` (or a gone row) drops the live-room map and broadcasts the terminal Error. A failed expiry comment is queued and the 5s expirer retries (edit the challenge comment when set). A busy match-row read on that first expiry comment or on the retry keeps the id queued — it is not treated as vanished. A decision abort (`no_conflicts` / `too_many` / `nothing`) that hits a busy write queues that body; the expirer aborts if still open and comments once. A busy `get_match` after that abort keeps the queued body (vanished still dequeues; abort_reason mismatch still dequeues). SHA-drift abort does the same: close the room, then comment. That rematch comment is only for a row whose stored `abort_reason` is `outdated` — a later finish or other close dequeues without posting. A busy abort write retries once, then a short background retry, instead of treating SQL error as already-closed. That background retry closes the room first (drift is known) and keeps retrying abort so rematch `/fight` is not stuck. The match id is latched as closing so Join cannot respawn lockstep while abort is still busy, and so clone-finish / expirer fight-link POSTs cannot publish a match URL after players already see `outdated`. Hunk insert and fighter-row update are open-status only so a closed row cannot gain rounds after abort. Shutdown drains sockets and does not expire a still-open row — outdated abort owns the rematch comment. That drain retries `get_match`: a known closed row uses its terminal Error, a still-open row stays `outdated` (close_room can land before abort SQL), vanished is `not found`, and a busy read is `preparing` so the canvas reconnects. The first terminal Error sticks on the canvas, so a busy drain must not invent rematch text. If that short loop still cannot abort, the id is queued in memory and the 5s expirer retries abort + the rematch comment. Restart loses that map; the next `synchronize` or 24h expiry covers a leftover drifted row that still has hunks. A busy `open_match_for_pr` on `synchronize` is not “no open match”: retry notice, do not `auto_challenge` while an open row may still exist. That lookup retry keeps going on the same backoff as abort, not four short attempts then silence. If the short loop still cannot read, the pull is queued and the 5s expirer keeps looking (same as abort). Closing a room does not hold the live-room map while Shutdown is delivered.
- Server restart: rooms rebuild from SQLite (`matches`, `match_inputs`, stored hunks). A pull-request match without hunks yet (clone still running, or clone failed and aborted) is not a room. A failed `match_inputs` read does not start confirm from an empty log — the room retries the load and withholds Hello until the durable ticks are replayed, so resume cannot diverge from the stored log. Clients reconnect and receive `Hello` at the latest confirmed tick. Finished GitHub matches with no result branch and no skip reason are published again (create-only; an existing git-fight ref for this match counts as done). A scored-open finish that cannot hash the last round retries instead of writing a fake `final_hash`. A busy input read is `Retry`. A stored winner whose inputs do not reach `sim.result` is `Unhashable`: Join and boot still retry (Error { preparing }), and the 5s expirer then marks `finished` without a `final_hash` so the one-open slot is released and picks can still publish. That hash uses the same contiguous input replay as resume, then `sim.forfeit` when the hunk is `forfeit_*`. A join while that finish is still retrying gets `Error { preparing }` and the socket closes (canvas reconnects) so it does not sit mute.
- A pending PR match with frozen SHAs and no hunks yet sends `Error { message: "preparing" }` and closes. The canvas keeps `preparing match…` (not `waiting for opponent`) and reconnects until hunks exist; the next socket then gets `Hello`. Spectator `Join` that cannot enter the room queue sends `Error { message: "busy" }` and closes. The canvas retries that the same way (`match busy — retrying`), without burning reconnects or reloading, so a teammate watching during a full queue is not kicked into a desync reload. `expired`, `aborted`, `finished`, and `not found` are terminal. The canvas ignores Hello/Tick/End and a later Error on a socket after a terminal Error or last-round End so a Shutdown `outdated` (busy `get_match` on drain) cannot replace expiry text or a replay URL. A terminal Error still wins over a Hello that has not marked the socket finished. The live-room map drops a sender only if it is still that room; a closed sender is not reused. Joining a match that just ended gets `Error`, not a new 30 Hz room. A busy `get_match` while sending that closed-room Error is `preparing`, not `finished`, so a SQLite blip cannot lock the canvas on a replay URL.

### Replay

A finished match already has `seed` and the full input log. `GET /replay/<id>` serves the client, which runs WASM locally feeding the log. No room, no inputs. `GET /ws?match=` on a finished match returns `Error { message: "finished" }` and does not spawn a room. After the last round the live room stops the 30 Hz clock and keeps existing sockets briefly so clients can read `End`; new sockets still get `Error`. Unfinished matches are not replayable. On restart, a live match whose current round already has a result in `match_inputs` runs that round's finish (next round or match over) instead of spawning a room that immediately exits. If every hunk already has a winner, the match is marked finished and published; it is not replayed from round 0. A scored-all live room retries that finish-only path. It only falls through to last-round `End { match_over }` when the current sim is already the last conflict. The 5s expirer does the same finish+publish for a scored open row that has no live room, and `expire_pending` / `expire_open_match` skip those rows. If that last-round hash is permanently uncomputable, the expirer finishes without inventing `final_hash` and still publishes picks. Boot and that tick also record any pending/in_progress/finished hunk whose `stats_recorded` is still 0. The same loop aborts leftover hunk-less GitHub rows older than the git job window, retries queued SHA-drift aborts, and posts a fight link for an open GitHub row that has hunks but no `challenge_comment_id`. That abort is one `UPDATE … RETURNING` that still requires no hunks, so a late `insert_hunks` cannot be killed as a failed clone.
